#!/bin/bash
# ============================================================
# Structora AI - PostgreSQL Auto-Setup Script (Root Version)
# ============================================================
# This script provisions and resets the PostgreSQL database and
# user role 'root' for Structora AI.
# ============================================================

echo "=== Structora AI Database Setup ==="
...
sudo -u postgres psql -c "SELECT 1 FROM pg_database WHERE datname = 'structora_db';" | grep -q 1
if [ $? -ne 0 ]; then
  sudo -u postgres psql -c "CREATE DATABASE structora_db OWNER root;"
  echo "✅ Created database 'structora_db'."
else
  echo "ℹ️ Database 'structora_db' already exists."
fi

# 4. Grant privileges
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE structora_db TO root;"
